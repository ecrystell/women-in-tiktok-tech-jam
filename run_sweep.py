...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        choices=("auto", "native"),
        default="auto",
        help="use measured dispatch decisions or force native attention",
    )
    parser.add_argument(
        "--skip-long",
        action="store_true",
        help="skip the final selected case (official ID 14 or integration S4096)",
    )
    ...
    if args.packed_sdpa_suffix_layers is not None and args.packed_sdpa_suffix_layers < 0:
    ...
    for case in cases:
        ...
        for process_index in range(processes):
            ...
            if completed.returncode != 0:
                if completed.returncode == 3 and "resource-blocked:" in completed.stdout:
                    resource_blocked = True
                    print(f"process {process_index + 1}: RESOURCE-BLOCKED")
